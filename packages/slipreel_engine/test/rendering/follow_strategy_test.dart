...
void main() {
  group('CenteredFollowStrategy', () {
    ...
    test('with followCursor=false returns the zoom rect centre, holding',
        () {
      final r = s.resolve(
    ...
    test('with cursor=null returns the zoom rect centre, holding', () {
      final r = s.resolve(
        zoom: _centered(),
        cursor: null,
        cursorVelocity: Offset.zero,
    ...
    test('with cursor set returns the cursor as the chase target', () {
      ...
      expect(r.isHolding, isFalse,
          reason: 'Cursor at a different position from the focal means the '
              'spring should be chasing, not holding');
    });
  });

  group('BoundedFollowStrategy gate semantics', () {
    test('with followCursor=false collapses to rect centre + hold', () {
    ...
    test(
      'engagement is strictly positional: cursor inside dz with zero '
      ...
      () {
        ...
        expect(r.target, focal,
            reason: 'Pin focal in place when cursor is inside the deadzone '
    ...
    test(
      'in-flight, cursor moving fast inside the deadzone keeps the gate '
    ...
    test(
      'release: in-flight cursor inside dz with velocity below threshold '
    ...
    test(
      'custom tuning: a higher cursorAtRest threshold releases the gate '
      ...
      () {
        ...
        expect(r.isHolding, isTrue,
            reason: 'A higher at-rest threshold accepts a faster cursor as '
                '"at rest"');
        expect(s.inFlight, isFalse);
      },
    );
