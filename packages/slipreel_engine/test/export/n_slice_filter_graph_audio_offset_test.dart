...
void main() {
  ...
  test(
    'a first slice trimmed past the gap needs no adelay (already aligned)',
    () {
  ...
  test('multi-slice: only the slice crossing stream start gets silence', () {
    final graph = buildExportFilterGraph(
      state: _stateWith([
    ...
    expect(
      'adelay'.allMatches(graph.filterComplex).length,
      1,
      reason: 'later slices begin after the stream is already present',
    );
    expect(graph.filterComplex, contains('adelay=240:all=1'));
    expect(
      graph.filterComplex,
  ...
  test('single track: slice silence is restored before concat', () {
    final graph = buildExportFilterGraph(
      state: _stateWith([_slice()]),
      audioStreams: [_sys(240000)],
    );
    expect(
      graph.filterComplex,
      contains('adelay=240:all=1,apad=whole_dur=10.000000'),
    );
    expect(graph.filterComplex, contains('[a_sys0]concat=n=1:v=0:a=1[outa]'));
  });

  test(
    'two tracks: each adelay feeds its track label, which feed the amix',
    () {
      final graph = buildExportFilterGraph(
        state: _stateWith([_slice()]),
        audioStreams: [
      ...
      expect(
        graph.filterComplex,
        contains('[a_mic0]concat=n=1:v=0:a=1[mic_track]'),
      );
      expect(
        graph.filterComplex,
        contains('[a_sys0]concat=n=1:v=0:a=1[sys_track]'),
      );
      expect(
        graph.filterComplex,
  ...
  test('two tracks with asymmetric gaps: only the gapped track is delayed', () {
    ...
    expect(
      graph.filterComplex,
      contains('concat=n=1:v=0:a=1[mic_track]'),
      reason: 'no-gap mic concats straight to its track, no adelay',
    );
    expect(
      graph.filterComplex,
      isNot(contains('a_mic_cat')),
  ...
  test(
    'a sub-millisecond gap rounds to 0 → no adelay (never emits adelay=0)',
    () {
      final graph = buildExportFilterGraph(
        state: _stateWith([_slice()]),
  ...
  test(
    'adelay coexists with a sped-up slice without disturbing its atempo',
    () {
      final graph = buildExportFilterGraph(
        state: _stateWith([
          ClipSlice(
      ...
      expect(
        ...
        contains('atempo='),
        reason:
            'per-slice speed handling is untouched by the track-level delay',
      );
      expect(
        graph.filterComplex,
        contains('adelay=120:all=1'),
        reason: 'the 240ms source gap is 120ms in a 2x edited slice',
      );
      expect(graph.filterComplex, contains('apad=whole_dur=5.000000'));
    },
  );

  test('slow motion expands leading silence in edited time', () {
    final graph = buildExportFilterGraph(
      state: _stateWith([
        ClipSlice(
          cutStart: Duration.zero,
    ...
    expect(graph.filterComplex, contains('adelay=480:all=1'));
    expect(graph.filterComplex, contains('apad=whole_dur=4.000000'));
  });

  test('slice wholly before audio start is padded to its full duration', () {
    final graph = buildExportFilterGraph(
      state: _stateWith([
        ClipSlice(
          cutStart: Duration.zero,
    ...
    expect(graph.filterComplex, contains('adelay=100:all=1'));
    expect(graph.filterComplex, contains('apad=whole_dur=0.100000'));
    expect(graph.filterComplex, contains('atrim=duration=0.100000'));
  });

  test(
    'decode seek offset shifts video trims but leaves audio source-time',
    () {
      final graph = buildExportFilterGraph(
        state: _stateWith([_slice(trimStart: const Duration(seconds: 1))]),
        audioStreams: [_sys(0)],
        videoTimeOffset: const Duration(milliseconds: 400),
      );
      expect(
        graph.filterComplex,
        contains('[0:v]trim=start=0.600000:end=9.600000'),
      );
      expect(
        graph.filterComplex,
        contains('[1:a:0]atrim=start=1.000000:end=10.000000'),
        reason: 'the separately opened audio input was not seeked',
      );
    },
  );
}
