...
void main() {
  ...
  test('FrameCompositor uses AccumulationCursorPainter, not the legacy '
      'CursorOverlayPainter (bug #3 — WYSIWYG between preview and export)', () {
    final source = File('lib/export/frame_compositor.dart').readAsStringSync();

    expect(
      source.contains('AccumulationCursorPainter'),
      isTrue,
      reason:
          'export must construct the same painter as the production '
    ...
    expect(
      ...
      isFalse,
      reason:
          'CursorOverlayPainter() construction in the export path '
          'reintroduces bug #3 (preview/export cursor-blur divergence). '
          'Use AccumulationCursorPainter to match production preview.',
    );
  });

  test('FrameCompositor exports the same idle reveal as preview', () {
    final source = File('lib/export/frame_compositor.dart').readAsStringSync();
    expect(source.contains('cursorRevealAt('), isTrue);
    expect(
      source.contains('visibilityReveal: cursorReveal'),
      isTrue,
      reason:
          'The export cursor painter must receive the deterministic '
          'fade-and-blur reveal used by PlaybackCanvas.',
    );
  });
}
