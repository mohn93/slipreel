...
- 2026-05-21 — Bug #6 fixed — `_ClipLocalState` placeholder dropped. `playbackSpeed`, `fadeIn`, `fadeOut` are now fields on `EditorProjectState` (defaults 1.0 / 0 / 0) with controller setters + JSON round-trip + copyWith. `ClipContextInspector` converted to `ConsumerWidget` reading from `editorProjectControllerProvider` — values persist across rebuilds and on disk. Pipeline still doesn't apply them (no per-clip speed/fade model yet) but the user no longer silently loses their picker choice. 2 new tests. **Bugs #7 and #10 investigated and not actioned**: #7 (deadzone anchor) — without a reproducible case the agent-proposed fix is speculative + this code path was just refactored under P1-5; #10 (canvas 100 ms vs focal 200 ms) — the two thresholds serve different purposes (pause-jitter floor vs. intentional-reverse-scrub detection), not actually a mismatch. Test result: 578 pass / 14 skip (+2 new).
