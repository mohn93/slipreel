...
/// triangle when the native monitor reports a real input failure. A valid
/// zero level is silence, not a failure; native liveness checks distinguish a
/// silent mic that is still delivering buffers from an engine/device problem.
...
class _MicStatusState extends State<MicStatus> {
  ...
  void _subscribe() {
    _sub = widget.levelStream.listen(
      (v) {
        if (!mounted) return;
        // Negative = native problem sentinel. Every valid 0..1 level,
        // including perfect digital silence, proves the input tap is alive.
        _setProblem(v < 0);
      },
      onError: (_, __) => _setProblem(true),
    );
  }
