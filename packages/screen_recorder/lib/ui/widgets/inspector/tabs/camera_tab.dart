...
/// Global camera LOOK controls (shape/roundness/mirror/border/shadow/opacity).
/// Reads/writes `cameraSettings` on the editor notifier. Per-region geometry
/// (position + size) is edited in the selected region's context inspector, not
/// here — each camera segment owns its own placement. Shows a disabled
/// placeholder when the recording has no camera sidecar.
