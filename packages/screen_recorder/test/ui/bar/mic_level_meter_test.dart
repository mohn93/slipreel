...
void main() {
  ...
  testWidgets('peak holds above the fill, then decays', (tester) async {
    ...
    c.add(1.0);
    await tester.pump();
    await pumpFrames(tester, 25); // ~400 ms — fill + peak near 1
    c.add(0.0);
    await tester.pump();
    await pumpFrames(tester, 8); // ~120 ms — fill springs down, peak holds
  ...
  testWidgets('peak band renders with non-zero size', (tester) async {
    ...
    c.add(1.0);
    await tester.pump();
    await pumpFrames(tester, 20); // peak rises off zero
  ...
  testWidgets('clamps out-of-range levels', (tester) async {
    ...
    c.add(5.0);
    await tester.pump();
    await pumpFrames(tester, 31); // ~500 ms of spring animation
