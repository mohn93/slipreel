...
void main() {
  ...
  testWidgets('shows Resume button when paused', (tester) async {
    await tester.pumpWidget(
      _h(status: RecordingStatus.paused, elapsed: const Duration(seconds: 5)),
    );
  ...
  testWidgets('labels the stop action Finish', (tester) async {
    await tester.pumpWidget(
      _h(
        status: RecordingStatus.recording,
        elapsed: const Duration(seconds: 5),
    ...
    expect(find.bySemanticsLabel(RegExp(r'^Finish recording')), findsOneWidget);
  });

  testWidgets('processing replaces recording controls with progress', (
    tester,
  ) async {
    await tester.pumpWidget(
      _h(
    ...
    expect(find.byKey(const Key('pill-stop')), findsNothing);
    expect(find.byKey(const Key('pill-pause')), findsNothing);
  });
