...
void main() {
  test('PlaybackCanvas defaults its stamp count to the shared engine '
      'constant', () {
    final source = File(
      'lib/ui/widgets/zoom/playback_canvas.dart',
    ).readAsStringSync();
    expect(
      source.contains('this.accumulationSampleCount = kCursorBlurStampCount'),
      isTrue,
      reason:
          'The preview stamp count must come from '
          'kCursorBlurStampCount so export (which reads the same '
          'constant) stays WYSIWYG.',
    );
  });

  test('PlaybackScreen passes the shared base exposure, not a literal', () {
    final source = File(
    ...
    expect(
      ...
      isTrue,
      reason:
          'The 150 ms virtual shutter must be single-sourced from '
    ...
    expect(
      ...
      isFalse,
      reason:
          'A literal exposure here can silently diverge from the '
          'export path.',
    );
  });

  test('PlaybackCanvas feeds the idle reveal into both cursor painters', () {
    final source = File(
      'lib/ui/widgets/zoom/playback_canvas.dart',
    ).readAsStringSync();
    expect(source.contains('cursorRevealAt('), isTrue);
    expect(
      RegExp(r'visibilityReveal:\s*cursorReveal').allMatches(source).length,
      2,
      reason:
          'Both accumulation and legacy preview painters must receive '
          'the same deterministic hide-when-idle reveal.',
    );
  });
}
