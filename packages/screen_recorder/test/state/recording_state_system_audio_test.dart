...
void main() {
  ...
  test('startRecording forwards the systemAudio config into settings', () async {
    ...
    const systemAudio =
        SystemAudioConfig(mode: SystemAudioMode.allApps);
    await c.startRecording(
      systemAudio: systemAudio,
    );
    await Future<void>.delayed(Duration.zero);

    expect(platform.capturedSettings?.systemAudio,
        const SystemAudioConfig(mode: SystemAudioMode.allApps));
    c.dispose();
  });

  test('startRecording with no systemAudio leaves settings.systemAudio null',
      () async {
    final platform = _CapturingPlatform();
    ScreenRecorderPlatform.instance = platform;
    final c = RecordingController();
    ...
    expect(platform.capturedSettings?.systemAudio, isNull);
    c.dispose();
  });
}
