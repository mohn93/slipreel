...
3. DONE: Created monthly product Apple ID `6811279465` in group `22378909`, with English US product localization. Review screenshot, subscription group localization and availability remain required. Complete paid-app agreements/tax/banking if Apple requires action.
4. Create/download the Mac App Store provisioning profile, using the existing Apple Distribution certificate for this team. Create an installer signing certificate only if packaging/upload requires it.
...
Not deployed or submitted. Deployed Apple credentials, production API authentication, provisioned signing, purchase lifecycle/sign-in acceptance, recording/export/folder-access acceptance and final metadata/media are release blockers.
