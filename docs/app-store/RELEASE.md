...
4. DONE: Generated and installed Mac App Store profile `Slipreel Mac App Store`, UUID `d3928fdc-491c-4352-b660-965df8890d05`, portal ID `AZHNTDN5YJ`, expires 2027-09-01. Verified bundle/team, Apple Sign In and matching installed distribution certificate. Mac Installer Distribution certificate is still needed for the upload package.
...
Installer CSR is prepared at `~/.config/slipreel/apple-store/mac-installer.certSigningRequest`, with its private RSA key stored owner-only beside it. Only the public CSR is copied to the preparation artifacts. No Mac Installer Distribution identity is installed; certificate generation and upload packaging remain pending. Browser tab control repeatedly timed out after the profile step. No upload, submission or deployment occurred.
