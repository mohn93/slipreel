...
3. DONE: Created monthly product Apple ID `6811279465` in group `22378909`, with English US product localization. English US subscription group localization is saved as Slipreel Pro, using app name Slipreel. Review screenshot remains required; monthly availability is now saved for all 175 storefronts. Complete paid-app agreements/tax/banking if Apple requires action.
...
- App Store Connect draft version is now 1.1.0, manual release, and the saved description includes monthly and yearly subscriptions. The existing Developer-role API key can read these records and validate packages; version and description were saved through the account-holder browser because Apple rejected API version edits with 403 and the group-level update with 409. Do not widen API permissions just for this task.

## Backend rollout preflight

PR 128 contains the release preparation. Backend CI passed, Linux CI passed, and the first macOS run passed the Dart suite but caught a website lint issue in the Apple subscription-management redirect. The account page now uses an ordinary Apple navigation link and hides the Stripe controls/caption for Apple billing; all website tests, SEO checks, network lint and deploy guards pass locally. CI is rerunning.

Production remains at `/opt/slipreel-api` on the established Slipreel VPS. Protected preflight database/environment backups are in `/var/backups/slipreel/store-rollout-20260912`. The candidate API is staged at `/opt/slipreel-api-store-71a63ed7`; all 85 file hashes match the locally built artifact. Apple keys are installed under `/etc/slipreel-apple` with root ownership, service-group access and restricted modes; production environment activation remains pending.

Migration 0009 was rehearsed on a restored, isolated database. Existing user/entitlement/device/session row counts were preserved; no missing or duplicate purchase UUIDs were found, and a second migration run applied nothing. The candidate service passed health/readiness, Apple challenge generation and unauthenticated/invalid-notification rejection checks on loopback port 18081. The previous API also passed readiness against the migrated clone, demonstrating code rollback compatibility. Both temporary services are stopped. The live service remains healthy. No test email, login, real purchase, App Store upload or review submission has occurred during this preflight.
