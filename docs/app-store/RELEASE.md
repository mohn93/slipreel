...
The Mac App Store distribution profile form is prepared for `Slipreel Mac App Store`, bundle `com.slipreel.store`, using existing Becoming Ventures Apple Distribution certificate `8ZF9CAH2WD` (expires 2027-09-01). Generation is awaiting action-time approval.

App Store Connect now has the promotional text, description, keywords and marketing URL saved as an unpublished draft. The separate distribution staging tree `/private/tmp/slipreel-store-distribution` is configured for the named profile and all ten workspace packages bootstrapped successfully. No signed distribution build has been produced from that tree yet.
