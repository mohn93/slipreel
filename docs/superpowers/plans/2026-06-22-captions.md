...
import 'package:slipreel_engine/captions/whisper_resolver.dart'; // not used directly; kept for cohesion
...
Note: the `whisper_resolver.dart` import above is unnecessary — remove it to avoid an unused-import warning. The needed imports are `dart:io`, `path`, `ffmpeg_probe`, `ffmpeg_resolver`, `caption_segment`. (`package:path` is already a transitive dep used across the engine; confirm with `melos run analyze`.)
...
Future<ui.Image> _render(CaptionStyle style) async {
  ...
  canvas.drawРect(Offset.zero & size, Paint()..color = const Color(0xFF335577));
...
Fix the deliberate typo before running: `drawРect` → `drawRect` (the `Р` is Cyrillic). Then generate the goldens:
